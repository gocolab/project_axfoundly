...
test.describe('E2E: 대시보드 인터랙션 및 관리자 기능 검증', () => {
  test('관리자 대시보드에서 회원 권한 변경 및 강좌 승인 탭이 동작한다', async ({ page }) => {
    ...
    // 4. 회원 관리 탭 및 강의/콘텐츠 관리 탭 확인
    await expect(page.getByRole('button', { name: '회원 관리' })).toBeVisible();
    await expect(page.getByRole('button', { name: '강의/콘텐츠 관리' })).toBeVisible();

    // 5. 강의/콘텐츠 관리 탭 선택
    await page.getByRole('button', { name: '강의/콘텐츠 관리' }).click();
    await expect(page.locator('text=개설 신청된 강의 검수')).toBeVisible();
  });
  ...
  test('강사 대시보드에서 [새 강의 등록] 버튼이 정상 렌더링된다', async ({ page }) => {
    ...
    const createCourseBtn = page.getByRole('button', { name: '새 강의 등록' });
