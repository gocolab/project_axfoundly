...
import type { IRProject, UserRole, IdeaRequest, IdeaProposal } from "../../types";
import Pagination from "../common/Pagination";
import SearchBar from "../common/SearchBar";
import { multiMatch } from "../../utils/searchUtils";
import { getDDayText } from "../IRPage";
import IdeaRequestModal from "../IdeaRequestModal";
import IdeaProposalModal from "../IdeaProposalModal";
import { useToast } from "../common/Toast";
import { api } from "../../lib/api";

export interface IdeaRequestsTabProps {
  initialIdeaRequests: IdeaRequest[];
  isLoggedIn: boolean;
  userRoles: UserRole[];
  userName?: string;
  projects: IRProject[];
  ...
  onSwitchToBrowseTab?: () => void;
  onSaveProject?: (project: IRProject) => void;
  selectedIdeaRequest?: IdeaRequest | null;
  onSelectIdeaRequest?: (req: IdeaRequest | null) => void;
}

export default function IdeaRequestsTab({
  initialIdeaRequests,
  isLoggedIn,
  userRoles,
  userName = "게스트",
  ...
  onSwitchToBrowseTab,
  onSaveProject,
  selectedIdeaRequest: controlledSelectedIdeaRequest,
  onSelectIdeaRequest: controlledOnSelectIdeaRequest,
}: IdeaRequestsTabProps) {
  const toast = useToast();
  const [ideaRequests, setIdeaRequests] = React.useState<IdeaRequest[]>(initialIdeaRequests);
  const [internalSelectedIdeaRequest, setInternalSelectedIdeaRequest] = React.useState<IdeaRequest | null>(null);

  // Synchronize controlled vs uncontrolled selectedIdeaRequest
  const selectedIdeaRequest = controlledSelectedIdeaRequest !== undefined
    ? controlledSelectedIdeaRequest
    : internalSelectedIdeaRequest;

  const setSelectedIdeaRequest = (req: IdeaRequest | null) => {
    if (controlledOnSelectIdeaRequest) {
      controlledOnSelectIdeaRequest(req);
    } else {
      setInternalSelectedIdeaRequest(req);
    }
  };

  React.useEffect(() => {
    if (initialIdeaRequests && initialIdeaRequests.length > 0) {
      setIdeaRequests(initialIdeaRequests);
    } else {
      fetchIdeaRequests();
    }
  }, [initialIdeaRequests]);
  ...
  const [ideaSort, setIdeaSort] = React.useState<"deadline" | "recent">("deadline");
  const [searchText, setSearchText] = React.useState<string>("");
  const [ideaPage, setIdeaPage] = React.useState<number>(1);
  const [ideasPerPage, setIdeasPerPage] = React.useState<number>(6);
  ...
  const fetchIdeaRequests = async () => {
    try {
      ...
      console.error("Fetch idea requests failed", err);
    }
  };
  ...
  const handleToggleSelectProposal = async (reqId: string, propId: string) => {
    ...
    try {
      ...
      if (res?.request) {
        ...
        if (selectedIdeaRequest?.id === reqId) {
          setSelectedIdeaRequest({
            ...selectedIdeaRequest,
            selectedProposalIds: updated,
            status: res.request.status,
            proposals: selectedIdeaRequest.proposals?.map((p) =>
              p.id === propId
  ...
  // Filter and sort idea requests
  const filteredIdeaRequests = React.useMemo(() => {
    let list = ideaRequests.filter((r) => {
    ...
    if (ideaSort === "deadline") {
      list = [...list].sort((a, b) => {
        const timeA = a.submissionDeadline ? new Date(a.submissionDeadline).getTime() : 9999999999999;
        const timeB = b.submissionDeadline ? new Date(b.submissionDeadline).getTime() : 9999999999999;
        return timeA - timeB;
      });
    } else {
      list = [...list].sort((a, b) => {
    ...
    return list;
  }, [ideaRequests, ideaStatusFilter, ideaSort, searchText]);
                          ...
                          <div className="flex items-center gap-2 flex-wrap mb-3 p-2 rounded-lg bg-black/20 border border-white/5">
                            ...
                            {prop.linkedProjectId && (
                              <button
                                onClick={() => {
                                  onSwitchToBrowseTab?.();
                                  onSelectProject?.(prop.linkedProjectId!);
                                }}
