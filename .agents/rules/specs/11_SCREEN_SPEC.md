...
| SCR-003 | 강의 목록 | `/courses` | 전체 | ✅ | 실시간 검색어 입력, 카테고리 필터, 카드별 원클릭 복사/수정, 강의 일정 표시, 페이지네이션 |
| SCR-004 | 강의 상세 | `/courses/:id` | 전체 | ✅ | 회차별 일시 커리큘럼, 원클릭 강의 복제 등록([📋 강의 복사]), 강의 달력 수강 신청, 강사 모달(진행강의/리뷰), 강사 인포그래픽 카드 |
| SCR-005 | 수강생 대시보드 | `/dashboard/student` | 수강생 | ✅ | 수강 강의(일정/진도), 결제 내역, 프로젝트/팀빌딩, 강사 메시지/알림함 |
| SCR-006 | 강사 대시보드 | `/dashboard/instructor` | 강사 | ✅ | AI 채팅 초벌 생성 모달, 내 강의 원클릭 복사 개강([📋 복사]), 징검다리 요일/시간 달력 연동 설정, 수강생 타깃 CRM 메시지 |
...
| SCR-010 | 커뮤니티 (멀티 게시판) | `/community`, `/community/:id` | 전체/회원 | ✅ | 검색어 필터, 게시판 분류(공지/팀빌딩/QnA), 글쓰기 모달, 2단 스플릿 상세 뷰, 공유 버튼, 페이지네이션 |
...
- **진행 방식 선택**: 실시간 온라인, 현장 오프라인, 온·오프라인 혼합 유형 중 선택. (※ **오프라인 강의장 주소 및 화상회의 링크 필드는 개설 모달에서 완전 삭제**되었으며, 수강 완료된 학생에게 별도 채널로 공지하는 정책 준수).
- **AI 채팅 초벌 & 커리큘럼 달력 연계 고도화**: 사용자 발화 원문을 단순 복사하지 않고 교육 기획 전문가 수준으로 항목을 판단·변환하여, 회차별 구체적 챕터 제목, 2~3문장 상세 실습 설명, 시작일 기준 요일 연계 미래 날짜를 자동 산출하여 상세 달력 폼에 실시간 동기화.
- **커리큘럼 자유 편집**: 회차 추가, 개별 삭제, 순서 이동(위/아래) 및 각 회차별 캘린더 날짜 피커(`input type="date"`)를 통해 회차별 강의일을 자유롭게 개별 지정.
- **수강생 탐색 단계**: 강의 상세 화면 및 목록 카드에서 진행 방식 뱃지 및 인터랙티브 달력 위젯에서 회차별 일정을 시각적으로 탐색하고 즉시 수강 신청.
...
### 3.9. 기존 강의 원클릭 복제 & 새 기수 개강 시스템 (Course Duplication System)
- **전사 통합 복제 진입점**:
  - 강의 상세 화면([`CoursePage.tsx`](file:///apps/project_launch_bizs/src/components/CoursePage.tsx)): 헤더 액션 영역에 `[📋 강의 복사]` 버튼 배치. 복제 완료 시 해당 신규 강의 상세로 즉시 전환.
  - 강사 대시보드([`InstructorDashboard.tsx`](file:///apps/project_launch_bizs/src/components/InstructorDashboard.tsx)): 내 강의 목록 각 행 카드에 `[📋 복사]` 버튼 배치.
  - 관리자 대시보드([`AdminDashboard.tsx`](file:///apps/project_launch_bizs/src/components/AdminDashboard.tsx)): 강의 관리 목록 및 상세 패널에 `[📋 복제]` 버튼 제공.
- **데이터 상속 및 초기화 격리 정책**:
  - 고유 ID 신규 발급, 제목 기수 자동 증가(예: "1기" ➔ "2기" 또는 "(새 기수)"), 개강 일정 자동 시프트(과거 날짜 배제, 차주 +7일 후 시작 및 기존 소요 기간/회차별 간격 유지).
  - 통계 리셋: 수강생 0명, 평점 5.0, 리뷰 0건 초기화, 상태 "모집중" 설정, 실시간 회의 링크 초기화.
...
- **회원 관리 (`/admin?tab=members`)**: 
  ...
  - 상세 패널 (2단 스플릿 뷰 - 4대 서브탭 구성):
    1. `기본 정보`: 접근 권한 다중 선택(체크박스 `member`, `manager`, `admin`), 최종 접속일 정보 확인, 회원 직권 강제 탈퇴(사유 입력 및 확인 모달 연동).
    2. `교육/강의 활동`: 회원이 개설한 강의 및 수강 신청한 강의 목록 열람.
    3. `스타트업/IR 활동`: 등록 IR 프로젝트, 아이디어 의뢰, 투자/협업 제안서 내역 열람.
    4. `강사 프로필 & 인증`:
       - **공식 인증 배지 관리 (관리자 직권 승인)**: 플랫폼 관리자가 직접 '공식 인증 배지 문구'(`certifiedBadge`)를 입력/저장/초기화. 실시간 배지 골드 엠블럼 미리보기 및 추천 프리셋 칩 제공.
       - **강사 프로필 & 실데이터 집계 현황**: 대표 직함, 상세 소개, 총 수강생(플랫폼 + 외부 수강생 `externalStudentCount`), 평균 평점, 강의 경력, 핵심 전문 역량 태그, 주요 경력 및 연혁(`careerHistory`) 타임라인 조회.
  - **강사 대시보드 연동 정책**: 강사 대시보드(`InstructorDashboard.tsx`)의 '공식 인증 배지 문구' 필드는 읽기 전용(`disabled`, 자물쇠 아이콘)으로 제한되며, 오직 관리자 대시보드에서만 승인 및 수정 가능.
- **GNB 상단 알림 드롭다운 & 알림센터 연동**:
  - 알림 펼침 팝업 상단 헤더 우측 및 하단 푸터에 `[전체 보기]` 링크를 제공하여, 클릭 시 마이페이지 알림센터(`/mypage?tab=notifications`)로 즉시 이동.
