...
### 3.5. 커뮤니티 마스터-디테일 스플릿 뷰 (Master-Detail Split View)
- **무깜박힘 반응형 분할**: 리스트 행 클릭 시 DOM 언마운트 없이 작성자/조회/날짜 메타 컬럼이 너비 0으로 부드럽게 축소되고, 좌측 목록이 50% 폭으로 줄어들며 우측에 상세 카드 및 댓글 패널이 슬라이드 인.
- **상세 탐색 & 단축키**: 상세 카드가 열린 상태에서 다른 행 클릭 시 부드러운 인라인 전환, `ESC` 키 입력 시 닫기 및 100% 폭 복원.
